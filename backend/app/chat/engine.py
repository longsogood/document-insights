...
def build_description_for_document(document: DocumentSchema) -> str:
    ...
    return "A document containing useful information about research paper that the user pre-selected to discuss with the assistant."
...
def get_tool_service_context(
    ...
    embedding_model = HuggingFaceEmbedding('dangvantuan/vietnamese-embedding')
...
async def get_chat_engine(
    ...
    for doc_id, index in doc_id_to_index.items():
        # storage_dir = os.path.join(index_dir, index_to_storage_name[i])
    ...
    merged_index = VectorStoreIndex(nodes=nodes,
                                    service_context=service_context)
    return merged_index.as_chat_engine(chat_mode='condense_plus_context')