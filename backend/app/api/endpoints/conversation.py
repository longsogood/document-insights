...
async def message_conversation(
    ...
    """
    # get conversation cùng với messeages
    conversation = await crud.fetch_conversation_with_messages(db, str(conversation_id))
    if conversation is None:
        raise HTTPException(status_code=404, detail="Conversation not found")
    # tạo user message với trạng thái SUCCESS
    ...
    # tạo một cặp kênh gửi và nhận để truyền thông điệp giữa các coroutine
    send_chan, recv_chan = anyio.create_memory_object_stream(100)

    # 
    async def event_publisher():
        async with send_chan:
            # tạo một async task để xử lý chat message
            task = asyncio.create_task(
                handle_chat_message(conversation, user_message, send_chan)
            )
            message_id = str(uuid4())
            # tạo một message object với role là chatbot và trạng thái là PENDING
            message = Message(
                id=message_id,
                conversation_id=conversation_id,
                content="",
